...
final public class GamepadA { //Why final? Because then Java would try to final every method it can, which is faster.
    ...
    //=============================
     ...
     public GamepadA(int xboxPort) {
        portNumber= xboxPort;
        CONTROLLER_MODE = Mode.GAMEPAD;
        xGP = new XboxController(portNumber);
    }
    
     public GamepadA(int jStickPort, int xPort, int yPort, int trigPort) {
